...
function payInterest12($connection, $sender, $mobile, $amount, $receiver, $applicationId, $id) {
	...
	if (!isMonth11Paid($connection, $id)) {
        header("location: ../payment?error=unpaid");
        exit();
    }
    ...
    if ($rowTerm['term'] === '12 Months') {
        $statusUpdate = "UPDATE financial_details SET status = 'Pending', updated_at = CURRENT_TIMESTAMP WHERE term = '12 Months' AND id = ?;";
	...
	$paymentUpdate = "UPDATE financial_details SET month_12 = 'Pending', updated_at = CURRENT_TIMESTAMP WHERE id = ?;";
...
function isMonth11Paid($connection, $id) {
    $query = "SELECT month_11 FROM financial_details WHERE id = ?";
    $stmt = mysqli_stmt_init($connection);
